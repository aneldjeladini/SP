...
bool IsValid (int n){
    if (reverseMiddle(n) % sumOuterNums(n) == 0 &&
        reverseMiddle(n)/ sumOuterNums(n)!=0){
        return true;
    }
    return false;
}
...
int main() {
    ...
    for (int i = from; i <=to ; ++i) {
        if (i>=100 && IsValid(i) ){
            cout << i << " -> " << "(" << reverseMiddle(i) << " == " << "("<<i%10<<" + "<<i/int((pow(10, digitNumber(i))/10))<<")"<<" * " << reverseMiddle(i)/sumOuterNums(i) << ")" << endl;
            count ++;
        }
    }
    cout << count << endl;
