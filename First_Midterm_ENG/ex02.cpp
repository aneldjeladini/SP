...
int FirstandLast (int n){
    ...
    border = border*10 + temp;
    return border;
}

bool isValid (int n){
    if (productMiddle(n) == 0){
        return false;
    }
    else if (productMiddle(n) % FirstandLast(n) == 0){
        return true;
    }
    return false;
}
...
int main() {
    ...
    for (int i = from; i <=to ; ++i) {
        if (isValid(i)){
            cout << i << " -> " << "(" << productMiddle(i) << " == " << FirstandLast(i) << " * " << productMiddle(i)/FirstandLast(i) << ")" << endl;
            count ++;
        }
    }
    cout << count << endl;
